    ...
    <main className="min-h-screen bg-[#171717] text-white">
      <div className="mx-auto flex min-h-screen max-w-md flex-col justify-center px-6">
        ...
        <div className="rounded-3xl border border-white/10 bg-[#212121] p-8 shadow-2xl">
          ...
          <h1 className="mt-2 text-4xl font-semibold">Sign up</h1>
          <p className="mt-2 text-sm text-slate-400">Create your SafeRoute account.</p>
          <form className="mt-8 space-y-4" onSubmit={(e) => e.preventDefault()}>
            <input type="text" placeholder="Name" className="h-12 w-full rounded-xl border border-white/10 bg-[#2a2a2a] px-4 outline-none focus:border-emerald-400" />
            ...
            <button className="h-12 w-full rounded-xl bg-[#10a37f] font-semibold text-[#10a37f]">Create account</button>
          </form>
          <button onClick={() => router.push("/signin")} className="mt-5 text-sm text-slate-400 hover:text-white">Already have an account? Sign in →</button>
        </div>
      </div>
    </main>
  );
